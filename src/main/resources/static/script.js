  ...
  try{
    ...
    propietarios.forEach((propietario) => {
      ...
      tr.innerHTML = `
        ...
        <td><button class="btn btn-sm btn-primary" onclick="verVehiculosDePropietario(${propietario.id})">Ver</button></td>
      `;
      tbody.appendChild(tr);
    });
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  btnPropietario.onclick = crearPropietario;
...
const { value: datosInspector } = await Swal.fire({
    title: 'Crear Inspector',
    html:
      '<input id="nombreInspector" class="swal2-input" placeholder="Nombre">' +
      '<input id="apellidoInspector" class="swal2-input" placeholder="Apellido">',
    focusConfirm: false,
    showCancelButton: true,
    preConfirm: () => {
      return {
        nombre: document.getElementById('nombreInspector').value,
        apellido: document.getElementById('apellidoInspector').value
      }
    }
  });
...
const { value: datos } = await Swal.fire({
    ...
    html:
      '<input id="nombreProp" class="swal2-input" placeholder="Nombre">' +
      '<input id="apellidoProp" class="swal2-input" placeholder="Apellido">' +
      '<input id="dniProp" class="swal2-input" placeholder="DNI">' +
      '<select id="esExento" class="swal2-input">' +
        '<option disabled selected>' + 'Es exento' + '</option>'  +
        '<option value="true">' + 'Si' + '</option>'  +
        '<option value="false">' + 'No' + '</option>'  + 
      '</select>' +
      '<input id="nacimientoProp" type="date" class="swal2-input" placeholder="Fecha de nacimiento">',
    focusConfirm: false,
    showCancelButton: true,
    preConfirm: () => {
      return {
        nombre: document.getElementById('nombreProp').value,
        apellido: document.getElementById('apellidoProp').value,
        dni: document.getElementById('dniProp').value,
        fechaNacimiento: document.getElementById('nacimientoProp').value
      }
    }
  });
  ...
  if (datos) {
    try {
      await fetch(`${API_BASE}/propietarios`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(datos)
      });
      Swal.fire('Creado!', 'Propietario creado correctamente.', 'success');
      cargarPropietarios();
    } catch (err) {
...
async function verVehiculosDePropietario(idPropietario) {
  try {
    const res = await fetch(`${API_BASE}/vehiculos/propietario/${idPropietario}`);
    const vehiculos = await res.json();

    if (vehiculos.length === 0) {
      Swal.fire('Sin vehículos', 'Este propietario no tiene vehículos registrados.', 'info');
      return;
    }

    const lista = vehiculos.map(v =>
      `<li><strong>${v.patente}</strong> - ${v.marca} ${v.modelo} (${v.anio})</li>`
    ).join('');

    await Swal.fire({
      title: 'Vehículos del Propietario',
      html: `<ul style="text-align:left;">${lista}</ul>`
    });
  } catch (err) {
    Swal.fire('Error', 'No se pudieron cargar los vehículos del propietario.', 'error');
  }
}